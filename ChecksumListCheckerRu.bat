	...
	while ($restart) {
		...
		if ($script:options.MissingEntries) {
			if ([string]::IsNullOrEmpty($script:options.EntMask)) {
				Write-Host '6.  Проверять отсутствующие записи по маске: Нет' -ForegroundColor DarkGray
			} elseif ($script:options.EntMaskInclude) {
				Write-Host ('6.  Проверять отсутствующие записи по маске (включить): ' + $script:options.EntMask)
			} else {
				Write-Host ('6.  Проверять отсутствующие записи по маске (исключить): ' + $script:options.EntMask)
			}
		} else {
			Write-Host '6.  Проверять отсутствующие записи по маске: Недоступно' -ForegroundColor DarkGray
		}
		if ($script:options.MissingEntries) {
			if ($script:options.EntTimeValue -gt 0) {
				Write-Host ('7.  Проверять отсутствующие записи по времени файла (' + (Get-TimeUnit $script:options.EntTimeUnit) + '): ' + $script:options.EntTimeValue)
			} else {
				Write-Host '7.  Проверять отсутствующие записи по времени файла: Нет' -ForegroundColor DarkGray
			}
		} else {
			Write-Host '7.  Проверять отсутствующие записи по времени файла: Недоступно' -ForegroundColor DarkGray
		}
		if ($script:options.VerifyHash -ne [VerifyMode]::No) {
			if ([string]::IsNullOrEmpty($script:options.HashMask)) {
				Write-Host '8.  Проверять хэши по маске: Нет' -ForegroundColor DarkGray
			} elseif ($script:options.HashMaskInclude) {
				Write-Host ('8.  Проверять хэши по маске (включить): ' + $script:options.HashMask)
			} else {
				Write-Host ('8.  Проверять хэши по маске (исключить): ' + $script:options.HashMask)
			}
		} else {
			Write-Host '8.  Проверять хэши по маске: Недоступно' -ForegroundColor DarkGray
		}
		if ($script:options.VerifyHash -ne [VerifyMode]::No) {
			if ($script:options.HashTimeValue -gt 0) {
				Write-Host ('9.  Проверять хэши по времени файла (' + (Get-TimeUnit $script:options.HashTimeUnit) + '): ' + $script:options.HashTimeValue)
			} else {
				Write-Host '9.  Проверять хэши по времени файла: Нет' -ForegroundColor DarkGray
			}
		} else {
			Write-Host '9.  Проверять хэши по времени файла: Недоступно' -ForegroundColor DarkGray
		}
		Write-Host ''
		...
		if ($script:options.MissingEntries) {
			if ($script:options.AbsPaths) {
				Write-Host '11. Тип путей отсутствующих записей: Абсолютные'
			} else {
				Write-Host '11. Тип путей отсутствующих записей: Относительные'
			}
		} else {
			Write-Host '11. Тип путей отсутствующих записей: Недоступно' -ForegroundColor DarkGray
		}
		if ($script:options.SysResolution) {
			Write-Host '12. Метод нормализации путей: Системный'
		} else {
			Write-Host '12. Метод нормализации путей: Внутренний'
		}
		Write-Host ''
		Write-Host '13. Список файлов контрольных сумм и путей поиска'
		Write-Host ''
		Write-Host '0.  Выход'
		Write-Host ''
...
function Show-MaskMenu {
	...
	try {
		...
		Write-Host ''
		$readValue = Read-Host 'Выберите режим маски (1-2)'
		switch ($readValue.Trim()) {
			'1' {
				Write-Host ''
...
function Show-PathList {
	...
	while ($restart) {
		...
		$msg = $true
		while ($msg) {
			Write-Host 'Задать новый список? (Да / ' -NoNewline
...
$newFiles = [ordered]@{}
$script:now = Get-Date
Write-Log 'Начало проверки' -NoHost
Write-Log '' -NoHost
Start-Clear
...
foreach ($file in $script:checkFiles) {
	...
	if (($script:options.MissingEntries -or ($script:options.VerifyHash -eq [VerifyMode]::Paths)) -and $script:auto) {
		...
		Write-Log ('Путь поиска: ' + (Remove-LongPrefix $script:checkPaths))
		if ($script:options.MissingEntries) { $newFiles = Get-NewFiles $script:checkPaths }
	}
	...
	if ($script:stats.FilesChecked -or $script:stats.FilesCheckedErr) {
		if ($script:stats.EntriesChecked) {
			...
			if ($script:stats.EntriesNotFound) {
				Write-Log ('Записи в файле контрольных сумм не найдены: ' + $script:stats.EntriesNotFound) -ForegroundColor Yellow
				$checkWarnings = $true
			}
		} else {
		...
		if ($script:stats.FilesNotFound) {
			if ($script:auto) {
				Write-Log ('Файлы по путям поиска не найдены: ' + $script:stats.FilesNotFound) -ForegroundColor Yellow
			} else {
				Write-Log 'Файлы по путям поиска не найдены!' -ForegroundColor Yellow
			}
			$checkWarnings = $true
	...
	if ($script:options.Update) {
		...
		foreach ($fileName in $updateObjects.Keys) {
				...
				if ($script:options.MissingEntries) {
					foreach ($entPath in $updObj.NewEntries.Keys) {
						...
						if (-not [string]::IsNullOrEmpty($hash)) {
							...
							if ($script:options.HashVerbose) { Write-Log ('Вычислен хэш: ' + $updObj.NewEntries[$entPath]) -ForegroundColor Green }
							$updObj.HashesAdded++
						} else {
							Write-Log ('Не удалось вычислить хэш: ' + $updObj.NewEntries[$entPath]) -ForegroundColor Red
			...
			if ($updObj.HashesAdded) {
				Write-Log ('Добавлено записей: ' + $updObj.HashesAdded)
				$script:stats.HashesAdded += $updObj.HashesAdded
			}
			if ($updObj.HashesNotAdded) {
				Write-Log ('Не добавлено записей: ' + $updObj.HashesNotAdded) -ForegroundColor Red
		...
		if ($updateObjects.Count -ne 1) {
			...
			if ($script:stats.HashesAdded) { Write-Log ('Добавлено записей: ' + $script:stats.HashesAdded) }
			if ($script:stats.HashesNotAdded) {
				Write-Log ('Не добавлено записей: ' + $script:stats.HashesNotAdded) -ForegroundColor Red
				$systemErrors = $true
			}
			if ($systemErrors) {
